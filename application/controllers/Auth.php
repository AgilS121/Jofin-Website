...
class Auth extends CI_Controller
{
	...
	public function registrasi()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$data['title'] = 'JOFIN Admin Registration';
			$this->load->view('templates/auth_header', $data);
            ...
            <div class="alert alert-success" role="alert">
             Registrasi berhasil. Silahkan Aktivasi terlebih dahulu
            </div>');
			redirect('auth');
		}
	}
	...
	private function _sendEmail($token, $type)
	{
		$config = [
			...
			'smtp_user' => 'kiravelnote@gmail.com',
			'smtp_pass' => 'WEB',
			'smtp_port' => 587,
			'mailtype' => 'html',
			'charset' => 'utf-8',
		...
		$this->email->from('kiravelnote@gmail.com', 'JOFIN Mobile');
		$this->email->to($this->input->post('email'));

		if ($type == 'verify') {
			$this->email->subject('Akun Verifikasi');
			$this->email->message('Klik Link untuk verifikasi akun : <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '& token=' . urlencode($token) . '">Activate</a>');
		} else if ($type == 'forgot') {
	...
	public function verify()
	{
		...
		$user = $this->db->get_where('tb_admin', [
			'email' => $email
		])->row_array();
		...
		if ($user) {
			...
			if ($user_token) {
				if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
					...
					$this->session->set_flashdata('msg', '
            <div class="alert alert-success" role="alert">' . $email . 'Telah di Aktivasi! Silahkan Login</div>');
					redirect('auth');
				} else {

					$this->db->delete('tb_admin', ['email' => $email]);
					$this->db->delete('tb_token', ['token' => $token]);
					$this->session->set_flashdata('msg', '
            <div class="alert alert-danger" role="alert">
            Aktivasi akun gagal! Token expire
            </div>');
					redirect('auth');
				}
			} else {
				$this->session->set_flashdata('msg', '
            <div class="alert alert-danger" role="alert">
            Aktivasi akun gagal! Salah token
            </div>');
				redirect('auth');
			}
		} else {
	...
	public function forgotPassword()
	{
		...
		Cek email untuk reset password
				...
				$this->session->set_flashdata('msg', '
		<div class="alert alert-danger" role="alert">
		Email tidak belum diregistrasi atau Aktivasi
		</div>');
				redirect('auth/forgotPassword');
			}
		}
	...
	public function changePassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->load->view('templates/auth_header', $data);
			$this->load->view('admin/changepwmail');
			$this->load->view('templates/auth_footer');
		} else {
